...
def render_video(story: dict) -> str | None:
    payload = {
        ...
        "text_fore_color": "#FFFF00",        # yellow text, no highlight box
...
def remux_faststart(video_path: str) -> str:
    ...
    output puts it at the end, which is fine for YouTube but can cause
    Instagram's ingestion to drop the audio track since it starts processing
    ...
    subprocess.run(
        [ffmpeg_exe, "-y", "-i", video_path, "-c", "copy", "-movflags", "+faststart", out_path],
